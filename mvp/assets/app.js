(function (root, factory) {
  ...
  function buildSavingsPlan({ intent, stores = [], offers = [], offerFreshness = 'unknown', storeError = null, offerError = null } = {}) {
    ...
    const offer = normalizedOffers[0] || null;
    ...
    if (offerFreshness !== 'fresh') blockers.push('offer_freshness_not_verified');
    ...
    return Object.freeze({
      ...
      otherOffers: Object.freeze(normalizedOffers.slice(1, 3)),
      offerFreshness,
...
(function (root, factory) {
  ...
  function createFindSavingsUseCase({ storeProvider, offerProvider } = {}) {
    ...
    return async function execute({ query, location } = {}) {
      ...
      const [storeResult, offerResult] = await Promise.allSettled([
...
(function (root, factory) {
  ...
  function reliabilityText(plan) {
    const parts = [];
    if (plan.store) parts.push('门店来自 OpenStreetMap，属于附近候选');
    ...
    return `
      <section class="result-card">
        ...
        <div class="answer-grid">
          ...
          <div class="answer-section savings"><div class="answer-label">可靠可省</div><div class="saving-value">待确认</div><div class="answer-sub">当前没有足够证据计算可靠金额</div></div>
          <div class="answer-section"><div class="answer-label">怎么省</div>${how}</div>
          <div class="answer-section"><div class="answer-label">是否可靠</div><div class="trust-copy">${escapeHtml(reliabilityText(plan))}</div><div class="badges"><span>门店：候选</span><span>金额：未知</span><span>叠加：未确认</span></div></div>
        </div>
        ${otherStores}
  ...
  return Object.freeze({ renderPlan, formatDistance, escapeHtml });
});
...
(function () {
  ...
  if (!S || !S.Decision || !S.BrowserLocation || !S.OsmStores || !S.PreviewOffers || !S.FindSavings || !S.Render) throw new Error('StackBack MVP modules are incomplete');

  const storeProvider = S.OsmStores.createStoreProvider();
  const offerProvider = S.PreviewOffers.createOfferProvider();
  const findSavings = S.FindSavings.createFindSavingsUseCase({ storeProvider, offerProvider });
  ...
  async function search() {
    ...
    setBusy(true);
    showMessage('正在同时找附近门店和可核验的官方优惠候选…');
    try {
